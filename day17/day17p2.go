		...
		for j := -1; j <= 1; j++ {
			for k := -1; k <= 1; k++ {
				for l := -1; l <= 1; l++ {
					...
					searchCube := cube{currentCube.x + i, currentCube.y + j, currentCube.z + k, currentCube.w + l}
		...
		for c := range activeCubes {
			for i := -1; i <= 1; i++ {
				for j := -1; j <= 1; j++ {
					for k := -1; k <= 1; k++ {
						for l := -1; l <= 1; l++ {
							searchCube := cube{c.x + i, c.y + j, c.z + k, c.w + l}
...
func main() {
	activeCubes := readActiveCubes("sample.txt")
